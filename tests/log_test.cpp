...
namespace {
  ...
  bool readsLogLevelEnvironment() {
    ...
    bool ok = expect(setenv("CHIROPTERA_LOG_LEVEL", "warn", 1) == 0, "failed to set CHIROPTERA_LOG_LEVEL");
    initLogLevelFromEnvironment();
    ok = expect(currentLogLevel() == LogLevel::Warn, "environment log level was not applied") && ok;

    setLogLevel(LogLevel::Error);
    ok = expect(setenv("CHIROPTERA_LOG_LEVEL", "WRN", 1) == 0, "failed to set invalid CHIROPTERA_LOG_LEVEL") && ok;
    initLogLevelFromEnvironment();
    ok = expect(currentLogLevel() == LogLevel::Error, "invalid environment log level changed current level") && ok;
    unsetenv("CHIROPTERA_LOG_LEVEL");
    return ok;
  }

  bool writesCappedLogLines() {
    const auto cacheRoot = makeTempRoot("chiroptera-log-cap");
    ...
    const auto logPath = cacheRoot / "chiroptera" / "chiroptera.log";
    std::error_code ec;
  ...
  bool rotatesWhileRunning() {
    const auto cacheRoot = makeTempRoot("chiroptera-log-rotate");
    if (!expect(useCacheHome(cacheRoot), "failed to set XDG_CACHE_HOME")) {
      return false;
    }

    const auto logDir = cacheRoot / "chiroptera";
    std::filesystem::create_directories(logDir);

    const auto logPath = logDir / "chiroptera.log";
    ...
    const auto backupPath = logDir / "chiroptera.log.1";
    std::error_code ec;
