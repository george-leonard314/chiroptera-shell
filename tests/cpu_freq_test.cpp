...
int main() {
  char dirTemplate[] = "/tmp/chiroptera_cpu_freq_test_XXXXXX";
  ...
  ok = expectNear(chiroptera::system::cpu_freq::readFreqs(root).curMhz.value_or(0.0), 2100.0, "average across cores")
      && ok;

  writeStat(root / "cpu0/cpufreq/scaling_cur_freq", "0");
  ok = expectNear(chiroptera::system::cpu_freq::readFreqs(root).curMhz.value_or(0.0), 2400.0, "zero entries skipped")
      && ok;

  const std::filesystem::path emptyRoot = root / "empty";
  std::filesystem::create_directory(emptyRoot);
  ok = expect(!chiroptera::system::cpu_freq::readFreqs(emptyRoot).curMhz.has_value(), "empty root yields nullopt") && ok;

  writeStat(root / "cpufreq/", ""); // stray non-cpuN dir at root must be ignored
  std::filesystem::create_directory(root / "offline");
  ok = expectNear(chiroptera::system::cpu_freq::readFreqs(root).curMhz.value_or(0.0), 2400.0, "stray dirs ignored") && ok;

  writeStat(root / "cpu0/cpufreq/scaling_max_freq", "4800000");
  ok = expectNear(chiroptera::system::cpu_freq::readFreqs(root).maxMhz, 4800.0, "max freq from scaling_max_freq") && ok;

  ok = expect(!chiroptera::system::cpu_freq::readFreqs(emptyRoot).maxMhz.has_value(), "no max freq yields nullopt") && ok;

  std::filesystem::remove_all(root / "cpu0/cpufreq");
  std::filesystem::remove_all(root / "cpu1/cpufreq");
  const auto unavailable = chiroptera::system::cpu_freq::readFreqs(root);
