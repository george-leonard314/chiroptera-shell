...
namespace {
  ...
  std::filesystem::path makeTempDir() {
    std::string pattern = (std::filesystem::temp_directory_path() / "chiroptera-plugin-git-export-XXXXXX").string();
...
int main() {
  ...
  ok = writeText(source / "clock/plugin.toml", "id = \"chiroptera/clock\"\nversion = \"1.0.0\"\nplugin_api = 3\n") && ok;
