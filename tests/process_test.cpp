...
namespace {
  ...
  bool syncAppliesEnvOverrides() {
    ::setenv("CHIROPTERA_PROCESS_UNSET_TEST", "parent", 1);

    process::RunOptions options;
    options.env.push_back({"CHIROPTERA_PROCESS_SET_TEST", "child"});
    options.env.push_back({"CHIROPTERA_PROCESS_UNSET_TEST", std::nullopt});

    const auto result = process::runSync(
        {"/bin/sh", "-lc", R"(printf '%s/%s' "$CHIROPTERA_PROCESS_SET_TEST" "${CHIROPTERA_PROCESS_UNSET_TEST-unset}")"},
        options
    );
    ::unsetenv("CHIROPTERA_PROCESS_UNSET_TEST");
  ...
  bool detachedAsyncInheritsLaunchEnvironment() {
    const std::filesystem::path outPath =
        std::filesystem::temp_directory_path() / ("chiroptera_process_env_test_" + std::to_string(::getpid()));
    std::error_code ec;
    std::filesystem::remove(outPath, ec);

    ::setenv("CHIROPTERA_WALLPAPER_PATH", "/tmp/chiroptera test/wallpaper.png", 1);
    ::setenv("CHIROPTERA_WALLPAPER_CONNECTOR", "DP-1", 1);

    const std::string command = R"(printf '%s\n%s' "$CHIROPTERA_WALLPAPER_PATH" "$CHIROPTERA_WALLPAPER_CONNECTOR" > )"
        + shellQuote(outPath.string());
    const bool launched = process::runAsync(command);
    ::unsetenv("CHIROPTERA_WALLPAPER_PATH");
    ::unsetenv("CHIROPTERA_WALLPAPER_CONNECTOR");
    ...
    for (int i = 0; i < 50; ++i) {
      ...
      if (contents == "/tmp/chiroptera test/wallpaper.png\nDP-1") {
        break;
      }
    ...
    std::filesystem::remove(outPath, ec);
    return expect(contents == "/tmp/chiroptera test/wallpaper.png\nDP-1", "detached async env was not visible in child");
  }
  ...
  bool cgroupDetectsSystemdUserManager() {
    ...
    ok = expect(
             process::cgroupIndicatesSystemdUserManager(
                 "0::/user.slice/user-1000.slice/user@1000.service/app.slice/chiroptera.service\n", 1000
             ),
             "chiroptera user service should be detected as user-manager managed"
         )
        && ok;
    ok = expect(
             !process::cgroupIndicatesSystemdUserManager(
                 "0::/user.slice/user-1000.slice/session-2.scope/chiroptera\n", 1000
             ),
    ...
    ok = expect(
             !process::cgroupIndicatesSystemdUserManager(
                 "0::/user.slice/user-1001.slice/user@1001.service/app.slice/chiroptera.service\n", 1000
             ),
    ...
    ok = expect(
             process::cgroupIndicatesSystemdUserManager(
                 "1:name=systemd:/user.slice/user-1000.slice/user@1000.service/app.slice/chiroptera.service\n", 1000
             ),
  ...
  bool detachedMissingBinaryReturnsFalse() {
    return expect(
        !process::runAsync(std::vector<std::string>{"/nonexistent/chiroptera-missing-binary-xyz"}),
