#!/usr/bin/env python3
"""
Analyze Chiroptera's C++ palette generator against Python + matugen references.
...
Three backends:
  - Chiroptera  : ../build-debug/chiroptera theme <img> --scheme <s> --dark
  - Python    : the upstream reference in chiroptera-shell/Scripts/python
...
CHIROPTERA_BIN = REPO_DIR / "build-debug" / "chiroptera"

# Absolute path into the upstream Python reference.
PYTHON_THEMING_DIR = Path.home() / "Development/misc/chiroptera/chiroptera-shell/Scripts/python/src/theming"
...
def run_chiroptera(image: Path, scheme: str) -> dict | None:
    try:
        out = subprocess.run(
            [str(CHIROPTERA_BIN), "theme", str(image),
             "--scheme", scheme, "--dark"],
            capture_output=True, text=True, check=True,
        )
        return json.loads(out.stdout)
    except Exception as e:
        print(f"  chiroptera {scheme}: {e}", file=sys.stderr)
        return None
...
def compare_m3(image: Path, scheme: str, has_matugen: bool) -> list[int]:
    ...
    noct = run_chiroptera(image, scheme)
    ...
    hdr = f"  {'Token':<26} {'Python':<10} {'Matugen':<10} {'Chiroptera':<10} {'Δ Py↔Noct':<14} {'Δ Mat↔Noct':<14}"
    print(hdr)
    print("  " + "─" * (len(hdr) - 2))
...
def compare_custom(image: Path, scheme: str) -> list[int]:
    ...
    hdr = f"  {'Token':<26} {'Python':<10} {'Chiroptera':<10} {'Δ Py↔Noct':<14}"
    print(hdr)
    print("  " + "─" * (len(hdr) - 2))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare Chiroptera C++ palette generator vs Python/matugen"
    )
    ...
    if not args.wallpaper.exists():
        ...
        return 1
    if not CHIROPTERA_BIN.exists():
        print(f"error: chiroptera not built: {CHIROPTERA_BIN}", file=sys.stderr)
        return 1
