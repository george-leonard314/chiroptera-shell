...
PYTHON_PROCESSOR = (
    Path.home() / "Development/misc/chiroptera/chiroptera-shell/Scripts/python/src/theming/template-processor.py"
)
PYTHON_THEMING_DIR = Path.home() / "Development/misc/chiroptera/chiroptera-shell/Scripts/python/src/theming"
TEMPLATE_ROOT = Path.home() / "Development/misc/chiroptera/chiroptera-shell/Assets/Templates"
PREDEFINED_SCHEME_JSON = (
    Path.home() / "Development/misc/chiroptera/chiroptera-shell/Assets/ColorScheme/Chiroptera-default/Chiroptera-default.json"
)
...
def main() -> int:
    ...
    if not CHIROPTERA_BIN.exists():
        print(f"error: chiroptera binary not found: {CHIROPTERA_BIN}", file=sys.stderr)
        return 1
