#!/usr/bin/env python3
"""
Static i18n checks for chiroptera.
...
SKIP_LINE_HINT = re.compile(
    r"(assert\s*\(|static_assert\s*\(|\bkLog\w*\.|\buiAssert|\bLogger\b|\bstd::cerr\b|\bspdlog::|\bCHIROPTERA_DEBUG\b|\bthrow\b)",
)
...
def is_placeholder_or_internal_literal(s: str) -> bool:
    """Non–user-copy literals (empty, gauges, paths, chiroptera: commands, etc.)."""
    t = s.strip()
    if not t:
        return True
    low = t.lower()
