...
A palette is the set of colors selected by [`[theme]`](/chiroptera/theming/). The theme config chooses the source and mode; the palette provides the actual color values used by bars, panels, widgets, templates, and other theme-aware UI.
...
Settings role pickers expose these role names first. Template files access these roles and the full Material token set through the [Template Reference](/chiroptera/theming/templates/#available-color-tokens).

## Custom Palette Files

When `source = "custom"`, `custom_palette` names a JSON file in the Chiroptera config directory's `palettes/` folder:
...
```text
~/.config/chiroptera/palettes/MyPalette.json
```
...
```text
~/.config/chiroptera/colorschemes/MyScheme/MyScheme.json
-> ~/.config/chiroptera/palettes/MyScheme.json
```

## Palette JSON

Each palette can provide `dark` and `light` variants. If `light` is omitted, Chiroptera uses the dark variant for both modes.
