...
:::note
Chiroptera uses `<* *>` for block tags and `{{ }}` for expressions. This is different from matugen, which uses Tera syntax (`{% %}` / `{{ }}`). The two are not interchangeable: a matugen template must be adapted to use `<* *>` blocks.
:::
...
In Chiroptera's main config, put the same custom color definitions under `[theme.templates.custom_colors]` and reference them from any enabled user template:
...
output_path = [
  "$XDG_CONFIG_HOME/qt5ct/colors/chiroptera.conf",
  "$XDG_CONFIG_HOME/qt6ct/colors/chiroptera.conf",
]
```
...
`post_action` selects a Chiroptera-owned native operation that runs after all outputs are written successfully. The
...
Template rendering is available through `chiroptera theme`.

Render one file:

```sh
chiroptera theme <image> -r input.txt:output.txt
```

Process a TOML config:

```sh
chiroptera theme <image> -c templates.toml
```
...
```sh
chiroptera theme <image> --builtin-config
```
...
```sh
chiroptera theme --theme-json theme.json -r input.txt:output.txt
```
...
```sh
chiroptera theme <image> -c templates.toml -r input.txt:output.txt
```
...
```sh
chiroptera theme <image> --default-mode light -r input.txt:output.txt
```
