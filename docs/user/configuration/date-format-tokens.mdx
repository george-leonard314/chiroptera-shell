...
Chiroptera date and time settings render the current local time. Clock widgets, `shell.time_format`, `shell.date_format`, and other date-format settings use the same token set when they need user-configurable date or time output.
...
Percent tokens use Chiroptera's strftime-compatible formatter, so localized names and locale-preferred formats follow `LC_TIME`, and timezone values follow the local system timezone. `%s` is handled by Chiroptera as Unix epoch seconds so it does not depend on platform-specific `strftime(3)` extensions.
