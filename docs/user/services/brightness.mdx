...
- `backlight_device` pins a monitor to a specific sysfs backlight device when a connector exposes more than one - accepts a bare name (`intel_backlight`) or a full path; run `chiroptera msg brightness-list-backlight-devices` to list the available device names.
...
[System Controls → Brightness](/chiroptera/ipc/system-controls/#brightness).
