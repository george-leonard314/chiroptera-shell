...
When disabled, Chiroptera keeps system-monitor UI available but stops the background sampling thread. Bar `sysmon` widgets, desktop `sysmon` widgets, and the Control Center System tab show unavailable values until the setting is enabled again.
...
When `cpu_temp_sensor_path` is set, Chiroptera uses only that file. If the path is missing, unreadable, not a `temp*_input` file, or cannot be parsed, CPU temperature is reported as unavailable until the configuration is fixed.
...
Chiroptera reads GPU statistics straight from the kernel and from the vendor's own management library where one exists. No elevated privileges are required, and Chiroptera does not invoke command-line monitoring tools such as `nvidia-smi`, `intel_gpu_top`, or `radeontop`.

::::note[Integrated GPUs are supported]
GPU monitoring is not limited to discrete GPUs. Chiroptera can report temperature and usage for supported integrated AMD and Intel GPUs when their kernel drivers expose those metrics. The `gpu_vram` stat is different: an integrated GPU that shares system memory has no dedicated VRAM to report, so that one stat is unavailable even though its other GPU stats can work.
::::
...
The open-source Nouveau driver does not provide NVML, so NVIDIA usage and VRAM monitoring require the proprietary driver. The `nvidia-smi` executable may be installed by the same package, but Chiroptera loads NVML directly and does not run it.
...
The value's color is interpolated between the widget's regular foreground color (its `color` setting) and its `highlight_color` (set per widget - see the [`sysmon` widget](/chiroptera/bar/widgets/sysmon/)). Below the activity threshold the value stays at its regular color; the moment it crosses the activity threshold it jumps to a small highlight tint, then ramps the rest of the way to the full `highlight_color`, reaching it at or above the critical threshold. Set the critical threshold to `0` to disable highlighting for that stat.
