...
Dock application icons follow the global `[shell.app_icon_colorize]` / `[shell.app_icon_color]` settings (see [Shell → App icon colorization](/chiroptera/configuration/shell/#app-icon-colorization)).

Configure in **Settings → Appearance → Interface**, or under `[shell]` in the Chiroptera TOML config stack.
...
[Surfaces → Dock](/chiroptera/ipc/surfaces/#dock).
