...
The Chiroptera launcher is a centralized search interface that provides quick access to your applications, system actions, and utility providers.

## How search works

As you type, Chiroptera queries all active **Launcher Providers**. Providers can either be "global" (always searching) or "prefixed" (triggered by a specific keyword).
...
- **Terminal apps**: Entries with `Terminal=true` run inside a terminal. Prefer `export TERMINAL=wezterm` (or another emulator on `PATH`); do not put `-e` in `$TERMINAL`. See [Shell: launching apps](/chiroptera/configuration/shell/#shell-settings) for discovery order and how this differs from `launch_apps_custom_command`.
...
`chiroptera dmenu` is a stdin/stdout picker for scripts that already produce newline-separated choices:

```sh
printf 'alpha\nbeta\ngamma\n' | chiroptera dmenu -p "Pick"
```
...
The command talks to the running Chiroptera instance on the same `XDG_RUNTIME_DIR` and `WAYLAND_DISPLAY`; start Chiroptera
first, then call `chiroptera dmenu` from scripts or compositor commands.
...
glyph = "terminal"
global = false
exec = "case \"{selection}\" in Open*) xdg-open ~/.config/chiroptera ;; Show*) notify-send \"$(date)\" ;; esac"
```
...
Chiroptera runs `exec` as a detached command; it does not automatically open a terminal. For interactive commands such as
...
(see [Shell settings](/chiroptera/configuration/shell/#shell-settings)).
...
See [Shell: Launcher settings](/chiroptera/configuration/shell/#shell-settings) for the full key reference, defaults, and examples.
