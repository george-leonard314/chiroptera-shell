...
| Form | Meaning |
|---|---|
| `<command> [arguments]` | Run an IPC command, exactly as `chiroptera msg` would |
...
The command vocabulary is the same one `chiroptera msg` uses, so anything you can bind to a keyboard
shortcut you can bind to a widget. Run `chiroptera msg --help` for the full list. The settings editor
