...
Application icons use global `[shell.app_icon_colorize]` when enabled (see [Shell → App icon colorization](/chiroptera/configuration/shell/#app-icon-colorization)).
...
**Pinned apps** (`pinned`) keep desktop entries on the taskbar when they are not running. Matching uses the same desktop-entry rules as the [dock](/chiroptera/dock/#pinned-matching). Pinned apps appear first (in list order), collapsed to one icon per app; other running windows follow as usual. Right-click an icon and choose **Pin** / **Unpin** to update the list. Non-running pins are dimmed by `pinned_opacity` (default `0.5`). Pins are only used on the flat strip; when `group_by_workspace` is on they are ignored (settings hide the list, and pin menu actions are unavailable).
