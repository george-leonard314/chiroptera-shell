...
System resource monitor. Shows a configurable combination of a glyph, visualization, and text value for one stat. Multiple instances with different stats can coexist on the same bar. Sampling cadence is controlled globally by [`[system.monitor]`](/chiroptera/services/system-monitor/), not per widget. Hovering the widget shows the selected stat first, followed by the other currently available sysmon stats.

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `stat` | string | `"cpu_usage"` | Which stat to display. See [Available stats](/chiroptera/services/system-monitor/#available-stats) |
...
| `glyph` | string | `""` | Custom bar glyph name (Chiroptera alias, Tabler icon name, or `U+` / `0x` codepoint). Leave empty to use the glyph for the selected stat |
...
| `highlight_color` | string | `"error"` | Color role (or fixed hex) the value tints toward as the stat rises past its activity/critical thresholds. See [System Monitor](/chiroptera/services/system-monitor/). |

The stats a `sysmon` widget can show, what each one displays, and where the values come from are listed under [Available stats](/chiroptera/services/system-monitor/#available-stats).
