...
Left-click cycles layouts using the compositor backend when supported. Bind `left` to an `exec` action to run a custom command instead - see [Widget Actions](/chiroptera/bar/actions/).
...
| `glyph` | string | `"keyboard"` | Bar glyph name: Chiroptera alias, Tabler glyph name, or `U+` / `0x` codepoint. Shown only when `show_glyph = true` |
...
[`[shell.keyboard_layout.custom_labels]`](/chiroptera/configuration/shell/#shell-settings). An alias overrides
