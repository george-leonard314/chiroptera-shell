...
|---------|------|---------|-------------|
| `glyph` | string | `"screenshot"` | Bar glyph name: Chiroptera alias, Tabler icon name, or `U+` / `0x` codepoint |
...
chiroptera msg screenshot-fullscreen
chiroptera msg screenshot-fullscreen pick
chiroptera msg screenshot-fullscreen DP-2
chiroptera msg screenshot-fullscreen all

```

Both commands use the global `[shell.screenshot]` output policy. The canonical command list is also under [Keybinds & IPC → Media & UI → Screenshots](/chiroptera/ipc/media-and-ui/#screenshots).
...
The bar widget’s fullscreen action matches `pick` on multi-monitor setups. To capture a specific display from a script without UI, pass the connector name (for example `chiroptera msg screenshot-fullscreen DP-1`).
