---
title: Neovim
description: Theming Neovim with Chiroptera
sidebar:
  order: 8
---

Neovim can be themed using the Chiroptera via the `base16` colorscheme.

## Setup

1. **Enable User Templates in Chiroptera:**
  - Open **Settings → Color Scheme → Templates → Advanced → Enable User 
   Templates**

2. **Add a user template for neovim**
  - Copy this into `~/.config/chiroptera/user-templates.toml`
...
5. **Setup base16 in your init.lua**
   ...
   ```lua
    require('matugen').setup()
   ```
When you restart neovim, it should use your chiroptera theme.
