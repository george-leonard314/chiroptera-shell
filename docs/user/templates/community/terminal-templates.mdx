...
1. Add this to your `~/.config/chiroptera/config.toml` (create it if you don't have one) :
    ``` toml
    [theme.templates.user.terminal-sequences]
    input_path = '~/.config/chiroptera/templates/terminal-sequences'
    output_path = '~/.cache/terminal-sequences'
...
2. Generate the template with this script:
    ...
    TEMPLATE_PATH=~/.config/chiroptera/templates/terminal-sequences

    mkdir -p ~/.config/chiroptera/templates
