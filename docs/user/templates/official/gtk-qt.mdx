...
GTK applications include most GNOME apps and many Linux applications. Chiroptera can automatically theme them using the `adw-gtk3` theme.
...
2. **Apply the GTK theme:**
   - **Using nwg-look (recommended):**
      :::tip[GTK4 Theming]
      In nwg-look, make sure to **uncheck the GTK4 option**. Leaving GTK4 checked can prevent GTK4 applications from using Chiroptera's color scheme.
...
3. **Enable GTK theming in Chiroptera:**
  - Open **Settings → Templates**
  - Toggle on **GTK 3** and/or **GTK 4**

Enabling those built-in templates writes `chiroptera.css` and runs the shipped
[`apply.sh`](https://github.com/george-leonard314/chiroptera-shell/blob/main/assets/templates/gtk/apply.sh)
`post_hook` (`bash '…/gtk/apply.sh' dark|light`). That script imports
`chiroptera.css` into `gtk.css` and syncs `adw-gtk3` / `adw-gtk3-dark` plus
...
[`theme_mode_changed`](/chiroptera/automation/hooks/) hook with `--appearance-only`
(skips the CSS import and does not change `gtk-theme`):

```toml
[hooks]
theme_mode_changed = "/usr/share/chiroptera/assets/templates/gtk/apply.sh --appearance-only $CHIROPTERA_THEME_MODE"
```

Use `/usr/local/share/chiroptera/...` instead if you installed with Meson’s default prefix.
`$CHIROPTERA_THEME_MODE` is set by the hook (`dark` or `light`). Switching
...
3. **Enable the KColorScheme (or QT theme) in Chiroptera:**
  - Open **Settings → Templates**
...
4. **Configure qt6ct:**
  ...
  - In the dropdown select `chiroptera (KColorScheme)` if using a KColorScheme for KDE app compatibility, or `chiroptera` for a normal QT theme without the need for KDE app compatibility.
  - Click **Apply** and **OK**
...
If you're using KDE Plasma, Chiroptera can apply colors through the native KColorScheme system.

### Setup

1. **Enable KColorScheme in Chiroptera:**
  - Open **Settings → Templates**
  - Toggle on **KColorScheme**

2. **Apply in KDE System Settings:**
  - Open **System Settings → Appearance → Colors**
  - Select the `chiroptera` color scheme
  - The scheme will update automatically when you change colors in Chiroptera

### Troubleshooting

- **Scheme not appearing:** Restart KDE Plasma or log out and back in
- **Colors not updating:** Ensure KColorScheme toggle is enabled in Chiroptera settings
