...
| Reload config | <IpcCommand command="config-reload" /> | Reload the merged Chiroptera config stack immediately. |
...
Set `CHIROPTERA_LOG_LEVEL` to `debug`, `info`, `warn`, or `error` before startup to choose the initial console log level. The default is `info`; the file log remains unfiltered.
...
```ini
hl.bind("ALT + TAB", hl.dsp.exec_cmd("chiroptera msg window-switcher"))
```
...
| Action | Command | Description |
|--------|---------|-------------|
| Lock session | <IpcCommand command="session lock" /> | Lock the current session. `loginctl lock-session` uses the same path when Chiroptera is running. |
