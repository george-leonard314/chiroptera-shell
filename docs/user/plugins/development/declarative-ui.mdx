...
as a retained `ui.*` tree; Chiroptera diffs each render against the previous tree and updates native controls in place.
...
```lua
local color = chiroptera.getConfig("color")
...
function ping()
  chiroptera.notify("Desktop widget", "button clicked")
end
```
...
| `ui.label` | `text`, `fontSize`, `color`, `fontWeight` (`thin`…`heavy`), `fontFamily` (a family name; load a file with `chiroptera.loadFont`), `baseline` (`text`/`textFixedHeight`/`inkCentered`/`pictographic`), `maxWidth`, `maxLines`, `textAlign` |
...
- **Images**: `ui.image` loads **local files only** - download remote previews with `chiroptera.download(url, dest, cb)`
...
- Ticks stay on `chiroptera.setUpdateInterval` - there is no frame-tick API in the bar.
...
`chiroptera ipc panel-close <id>` dismisses it. Several persistent panels can be open at once, alongside a normal
...
invented. Chord syntax is the same as elsewhere in Chiroptera (`space`, `ctrl+r`, `shift+Return`).
...
- **Escape always dismisses.** The panel-close action stays with the host and cannot be captured, so a panel can never
  ...
  [Cancel keybind](/chiroptera/configuration/shell/#keybinds), so rebinding it keeps working.
- **A captured chord outranks the shell navigation keybinds, inside that panel only.** Capture is checked before the
  keybind actions in [`[keybinds]`](/chiroptera/configuration/shell/#keybinds) are applied, so `capture_keys = ["space"]` means
...
From a widget or shortcut script, toggle a panel by its full entry id with `chiroptera.togglePanel("author/plugin:panel")`.
The same panel can also be opened, closed, or toggled externally over IPC:

```sh
chiroptera msg panel-toggle noctalia/example:panel
```
...
download + apply via `chiroptera.wallpaperDirectory()` and `chiroptera.setWallpaper()`).
