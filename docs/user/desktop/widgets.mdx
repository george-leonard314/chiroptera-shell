...
Desktop widgets are configured through the normal Chiroptera config stack. Declare layouts in `~/.config/chiroptera/*.toml`;
the interactive editor writes its layout overrides to `settings.toml`.

- Declarative config: `[desktop_widgets]` in `~/.config/chiroptera/*.toml`
- GUI/editor overrides: `$CHIROPTERA_STATE_HOME/chiroptera/settings.toml`, `$XDG_STATE_HOME/chiroptera/settings.toml`, or `~/.local/state/chiroptera/settings.toml`
...
When enabled, Chiroptera renders each desktop widget as its own tightly-sized layer-shell surface on the `Bottom` layer. The current implementation ships `button`, `calendar`, `clock`, `label`, `audio_visualizer`, `fancy_audio_visualizer`, `sticker`, `weather`, `media_player`, `volume`, and `sysmon` widget types plus an interactive edit mode.
...
```sh
chiroptera msg desktop-widgets-edit
chiroptera msg desktop-widgets-exit
chiroptera msg desktop-widgets-toggle-edit
```

You can also turn edit mode on from **Settings → Desktop → Toggle Editor**. Chiroptera shows a short internal notification and you should switch to an **empty workspace** (no windows covering the desktop) so the editor overlay is visible.
...
| `format` | string | `"{:%H:%M}"` | Digital clock format string. Ignored when `clock_style = "analog"`. Supports the same syntax and tokens as the bar clock `format`; see [Date format tokens](/chiroptera/configuration/date-format-tokens/). |
...
The calendar widget uses the configured [calendar service](/chiroptera/services/calendar/) accounts. It shows event indicators
...
The event list uses the shared `event_date_format` and `event_time_format` from [`[calendar]`](/chiroptera/services/calendar/),
...
Draws the current weather glyph alongside the temperature and short condition, driven by the shared `WeatherService`. Requires `[weather] enabled = true` in `config.toml` (see [Services - Weather](/chiroptera/services/weather/)) - the widget will render a placeholder otherwise.
...
System monitor widget with graph or pill-gauge display modes. Uses the shared `SystemMonitorService`. Poll intervals are configured globally under [`[system.monitor]`](/chiroptera/services/system-monitor/), not in the widget settings below.
...
| `stat` | string | `"cpu_usage"` | Primary stat. See [Available stats](/chiroptera/services/system-monitor/#available-stats) |
...
The stats a `sysmon` widget can show, what each one displays, and where the values come from are listed under [Available stats](/chiroptera/services/system-monitor/#available-stats). Desktop widgets support CPU frequency and every other stat except `ram_used` and the four disk stats.
