...
namespace {
  ...
  constexpr const char* kLogLevelEnv = "CHIROPTERA_LOG_LEVEL";
...
void initLogFile() {
  ...
  if (cacheHome != nullptr && cacheHome[0] != '\0') {
    dir = std::string(cacheHome) + "/chiroptera";
  } else if (home != nullptr && home[0] != '\0') {
    dir = std::string(home) + "/.cache/chiroptera";
  } else {
  ...
  const std::string logPath = dir + "/chiroptera.log";
  const std::string backupPath = dir + "/chiroptera.log.1";
