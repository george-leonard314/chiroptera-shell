...
namespace FileUtils {
  ...
  [[nodiscard]] inline std::string configDir() {
    const char* chiroptera = std::getenv("CHIROPTERA_CONFIG_HOME");
    if (chiroptera != nullptr && chiroptera[0] != '\0') {
      return std::string(chiroptera) + "/chiroptera";
    }
    const char* xdg = std::getenv("XDG_CONFIG_HOME");
    if (xdg != nullptr && xdg[0] != '\0') {
      return std::string(xdg) + "/chiroptera";
    }
    const char* home = std::getenv("HOME");
    if (home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.config/chiroptera";
    }
    return {};
  }

  [[nodiscard]] inline std::string stateDir() {
    const char* chiroptera = std::getenv("CHIROPTERA_STATE_HOME");
    if (chiroptera != nullptr && chiroptera[0] != '\0') {
      return std::string(chiroptera) + "/chiroptera";
    }
    ...
    const char* home = std::getenv("HOME");
    if (home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.local/state/chiroptera";
    }
    return {};
  }

  [[nodiscard]] inline std::string dataDir() {
    const char* chiroptera = std::getenv("CHIROPTERA_DATA_HOME");
    if (chiroptera != nullptr && chiroptera[0] != '\0') {
      return std::string(chiroptera) + "/chiroptera";
    }
    ...
    const char* home = std::getenv("HOME");
    if (home != nullptr && home[0] != '\0') {
      return std::string(home) + "/.local/share/chiroptera";
    }
    return {};
  }
  ...
  // materialized runtime dir) and honors the CHIROPTERA_STATE_HOME override.
