...
void LockscreenWidgetsController::registerIpc(IpcService& ipc) {
  ipc.bind(chiroptera::cli::msg::lockscreenWidgetsEdit, [this](const std::string&) -> std::string {
  ...
  ipc.bind(chiroptera::cli::msg::lockscreenWidgetsExit, [this](const std::string&) -> std::string {
    exitEdit();
    return "ok\n";
  });

  ipc.bind(chiroptera::cli::msg::lockscreenWidgetsToggleEdit, [this](const std::string&) -> std::string {
    if (m_config != nullptr && !m_config->isLockScreenEnabled()) {
      return "error: lock screen disabled\n";
    }
    toggleEdit();
