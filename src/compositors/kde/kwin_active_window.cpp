...
namespace {
  ...
  const sdbus::ServiceName kBusName{"dev.chiroptera.KWinActiveWindow"};
  const sdbus::ObjectPath kObjectPath{"/dev/chiroptera/KWinActiveWindow"};
  constexpr auto kInterface = "dev.chiroptera.KWinActiveWindow";
  constexpr auto kScriptLabel = "chiroptera-active-window";
...
const BUS = "dev.chiroptera.KWinActiveWindow";
const PATH = "/dev/chiroptera/KWinActiveWindow";
const IFACE = "dev.chiroptera.KWinActiveWindow";
...
function notify(window) {
  try {
    ...
    print("chiroptera-active-window notify failed: " + error);
  }
}

function isChiropteraShellSurface(window) {
  const resourceClass = (window.resourceClass || "").toLowerCase();
  if (resourceClass === "chiroptera") {
    return true;
  }
  const caption = (window.caption || "").toLowerCase();
  return caption === "chiroptera" && resourceClass === "";
}
...
function shouldTrack(window) {
  ...
  if (isChiropteraShellSurface(window)) {
    return false;
  }
  return true;
}
...
function syncWindows() {
  try {
    ...
    print("chiroptera-active-window sync failed: " + error);
  }
}
...
syncWindows();
  ...
  [[nodiscard]] bool isChiropteraShellSurface(const std::string& appId, const std::string& title) {
    const std::string appLower = StringUtils::toLower(appId);
    if (appLower == "chiroptera") {
      return true;
    }
    if (appLower.empty() && StringUtils::toLower(title) == "chiroptera") {
      return true;
    }
    return false;
  }
...
namespace compositors::kde {
  ...
  std::vector<std::string> KwinActiveWindow::runningAppIds() const {
    ...
    for (const auto& window : m_trackedWindows) {
      if (window.appId.empty()
          || isChiropteraShellSurface(window.appId, window.title)
  ...
  KwinActiveWindow::windowsForApp(const std::string& idLower, const std::string& wmClassLower) const {
    ...
    for (const auto& window : m_trackedWindows) {
      if (window.appId.empty() || isChiropteraShellSurface(window.appId, window.title)) {
        continue;
      }
  ...
  std::vector<WorkspaceWindow> KwinActiveWindow::trackedWorkspaceWindows() const {
    ...
    for (const auto& window : m_trackedWindows) {
      ...
      if (appId.empty() || isChiropteraShellSurface(appId, title)) {
        continue;
      }
...
for (const window of workspace.windowList()) {{
  ...
  if (window.resourceClass === targetClass && window.caption === targetCaption) {{
    ...
    const std::string label = std::format("chiroptera-activate-{}", ++m_transientScriptSerial);
...
for (const window of workspace.windowList()) {{
  ...
  break;
    ...
    const std::string label = std::format("chiroptera-close-{}", ++m_transientScriptSerial);
    if (!runTransientScript(script, label)) {
  ...
  void KwinActiveWindow::onNotifyActiveWindow(
    ...
    if (caption.empty() && resourceClass.empty()) {
      ...
      if (isChiropteraShellSurface(appId, title)) {
        return;
      }
  ...
  void KwinActiveWindow::onNotifyWindowList(const std::string& payload) {
    ...
    for (const auto& record : splitString(payload, kRecordSeparator)) {
      ...
      if (isChiropteraShellSurface(window.appId, window.title)) {
        continue;
      }
