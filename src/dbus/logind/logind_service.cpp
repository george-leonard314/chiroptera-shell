...
bool LogindService::acquireIdleInhibit() {
  ...
  try {
    ...
    m_managerProxy->callMethod("Inhibit")
        .onInterface(kLogindManagerInterface)
        .withArguments(std::string("idle"), std::string("chiroptera"), std::string("Caffeine"), std::string("block"))
        .storeResultsTo(fd);
...
bool LogindService::acquireSleepDelayInhibit() {
  ...
  try {
    ...
    m_managerProxy->callMethod("Inhibit")
        .onInterface(kLogindManagerInterface)
        .withArguments(
            std::string("sleep"), std::string("chiroptera"), std::string("Lock before sleep"), std::string("delay")
        )
        .storeResultsTo(fd);
