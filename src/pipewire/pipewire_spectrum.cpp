...
bool PipeWireSpectrum::Stream::start() {
  ...
  auto* props = pw_properties_new(
      PW_KEY_MEDIA_TYPE, "Audio", PW_KEY_MEDIA_CATEGORY, "Capture", PW_KEY_MEDIA_NAME, "Chiroptera Spectrum",
      PW_KEY_APP_NAME, "Chiroptera Spectrum", PW_KEY_STREAM_MONITOR, "true", PW_KEY_STREAM_CAPTURE_SINK, "true",
  ...
  m_stream = pw_stream_new(core, "chiroptera-spectrum", props);
  if (m_stream == nullptr) {
...
PipeWireSpectrum::PipeWireSpectrum(PipeWireService& service) : m_service(service) {
  m_diagEnabled = std::getenv("CHIROPTERA_SPECTRUM_DEBUG") != nullptr;
  initProcessing();
}
