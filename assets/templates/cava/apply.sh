...
if grep -q '^\[color\]' "$config_file"; then
    if sed -n '/^\[color\]/,/^\[/p' "$config_file" | grep -qE '^theme\s*=\s*"chiroptera"'; then
        :
    elif sed -n '/^\[color\]/,/^\[/p' "$config_file" | grep -qE '^theme\s*='; then
        tmp_file="$(mktemp "${config_file}.tmp.XXXXXX")"
        sed -E '/^\[color\]/,/^\[/{s/^theme\s*=.*/theme = "chiroptera"/}' "$config_file" >"$tmp_file"
        write_if_changed "$config_file" "$tmp_file"
    else
        tmp_file="$(mktemp "${config_file}.tmp.XXXXXX")"
        sed '/^\[color\]/a theme = "chiroptera"' "$config_file" >"$tmp_file"
        write_if_changed "$config_file" "$tmp_file"
    fi
else
    printf '\n[color]\ntheme = "chiroptera"\n' >>"$config_file"
